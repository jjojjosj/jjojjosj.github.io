...
test.describe("Blog Navigation and Pages", () => {
  test("홈페이지가 로드되고 포스트 목록이 표시됨", async ({ page }) => {
    ...
    // 타이틀 확인
    await expect(page).toHaveTitle(/컴저씨 블로그/);

    // 헤더 확인
    await expect(page.locator("header")).toBeVisible();
    await expect(
      page.getByRole("link", { name: "컴저씨 블로그" }),
    ).toBeVisible();
    ...
    // 사이드바 확인
    await expect(
      page.getByText("개발을 취미(목표)로 하는 인프라 엔지니어"),
    ).toBeVisible();
  ...
  test("포스트 상세 페이지가 제대로 로드됨", async ({ page }) => {
    ...
    // 포스트 내용 확인
    await expect(
      page.locator('article .prose, article [class*="prose"]'),
    ).toBeVisible();
  });
